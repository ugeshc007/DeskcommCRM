...
| Lead/Event ID and repeat customer | The implementation branch uses the existing lead UUID as the Event ID and enforces one open event per normalized international phone per organization, preserving closed history. | Confirm any desired shorter display format; keep the UUID as the canonical ID. |
