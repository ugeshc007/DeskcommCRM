...
export function FieldLiveView({ organizationId, userId, initialEmployeeId, initialDate, embedded = false }: { organizationId: string; userId: string; initialEmployeeId?: string; initialDate?: string; embedded?: boolean }) {
  ...
  return <Container className={embedded ? 'min-w-0 space-y-5' : 'mx-auto max-w-[1600px] space-y-5 p-4 md:p-6'}>
    ...
    {data && <>
      ...
      <p className="text-xs text-muted-foreground">Live pins appear only for online, on-duty phones and show their last reliable fix, with its time and accuracy. Orange means a newer GPS report was too imprecise or untrusted; the pin may be old. Offline officers remain in the cards with their last known GPS time. The shaded circle shows reported uncertainty. The blue path is recorded GPS, not a road-routed estimate. Tracking stops at punch-out.</p>
