      ...
      <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-3xl">
        ...
        {receipt ? <div className="space-y-3" role="status">
          ...
          <fieldset disabled={busy} className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="block space-y-1 text-sm">Messaging channel<select className={selectClass} required value={channel} onChange={e => setChannel(e.target.value)}><option value="">Choose a channel</option>{setup.channels.map((session, i) => <option key={session.id} value={session.id}>{session.display_name || `Messaging channel ${i + 1}`}</option>)}</select></label>
