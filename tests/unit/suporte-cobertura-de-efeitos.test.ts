...
it("todo handler mutante do app declara guarda de suporte ou é infraestrutura identificada",()=>{
 ...
 for(const path of files("app/api/v1").filter(p=>p.endsWith("/route.ts"))){
  ...
  if(path==="app/api/v1/field-sales/voice/route.ts")continue; // bearer próprio do aparelho; só transcreve áudio após validar org, empregado e turno ativo, sem cookie de suporte
  if(path==="app/api/v1/field-sales/pair/route.ts")continue; // sem cookie: código único de 5 minutos, emitido só por conta autorizada, troca atômica por bearer; suporte é barrado na emissão
