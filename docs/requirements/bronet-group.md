...
The customer confirmed that manager escalation starts **after one working day overdue**. The definition of a working day (organization calendar, holidays and cut-off time) still needs agreement before implementation.
...
| Campaign and performance reports | Meta Ads campaign read APIs and an activity report exist. | Add campaign-to-lead attribution and the requested qualified/follow-up/conversion/value, CRE × salesperson, and status-aging reports. Define qualification and CRE before calculating totals. |
...
6. Which manager receives the escalation, what calendar defines the agreed one-working-day threshold, and which notification channel is used?
7. For cross-brand assignment, is the source lead moved, copied, or kept with a linked counterpart? Who may authorize it in each organization?
