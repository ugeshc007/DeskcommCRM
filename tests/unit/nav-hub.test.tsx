...
describe("NavHub", () => {
  it("apresenta a IA nas três etapas da jornada, na ordem", () => {
    render(<NavHub locale="pt-BR" group="ia" isPlatformAdmin role={null} title="Agente de IA" subtitle="" />);
  ...
  it("some com a seção inteira quando a permissão esvazia", () => {
    render(
      <NavHub locale="pt-BR" group="organizacao" isPlatformAdmin={false} role="viewer" title="Org" subtitle="" />,
    );
    const secoes = screen.getAllByRole("heading", { level: 2 }).map((h) => h.textContent?.trim());
